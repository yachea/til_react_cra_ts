...
- 파일명.js : 변수, 함수 등을 작성함
- 파일명.jsx : 컴포넌트를 즉, HTML을 리턴함. (js로 해도됨)
- 파일명.ts : 변수, 함수 등을 작성함
...
// as 문법은 개발자가 HTMLement 라고 확신한다는 것을 VSCode 에 알려줌.
// as 문법은 개발작 null 이 아니라고 VSCode에 알려줌.
...
- `rfce` 탭 : React Funcion Component Export
...
- `rafce` 탭 : React Arrow Funcion Component Export
...
const App = (): JSX.Element => {
  return (
    <div>
      <h1>App</h1>
      <Sample title="Props 전달된 title 입니다."></Sample>
      <p>Children 입니다.</p>
    </div>
  );
};
